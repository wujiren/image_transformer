...
class ImageTransformer(nn.Module):
    ...
    def __init__(self, hparams):
        ...
        self.layers = nn.ModuleList(
            [DecoderLayer(hparams) for _ in range(hparams.nlayers)]
        )
        self.input_dropout = nn.Dropout(p=hparams.dropout)
        if (
            self.hparams.distr == "dmol"
        ):  # Discretized mixture of logistic, for ordinal valued inputs
            assert self.hparams.channels == 3, "Only supports 3 channels for DML"
            size = (1, self.hparams.channels)
            self.embedding_conv = nn.Conv2d(
                1, self.hparams.hidden_size, kernel_size=size, stride=size
            )
            ...
            self.embeds = nn.Embedding(
                NUM_PIXELS * self.hparams.channels, self.hparams.hidden_size
            )
            self.output_dense = nn.Linear(
                self.hparams.hidden_size, NUM_PIXELS, bias=True
            )
        else:
    ...
    def forward(self, X, sampling=False):
        ...
        for layer in self.layers:
            X = layer(X)
        X = self.layers[-1].preprocess_(
            X
        )  # NOTE: this is identity (exists to replicate tensorflow code)
        ...
        if not sampling and self.hparams.distr == "cat":  # Unpack the channels
            X = X.view(
                X.shape[0],
                X.shape[1],
                X.shape[2] // self.hparams.channels,
    ...
    def split_to_dml_params(self, preds, targets=None, sampling=False):
        ...
        mix_logits, locs, log_scales, coeffs = torch.split(
            preds, [nm, nm * 3, nm * 3, nm * 3], dim=-1
        )
    ...
    def loss(self, preds, targets):
        if self.hparams.distr == "dmol":
            ...
            ce = F.cross_entropy(
                preds.permute(0, 4, 1, 2, 3), targets, reduction="none"
            )
            return ce
    ...
    def accuracy(self, preds, targets):
        if self.hparams.distr == "cat":
            ...
            acc = torch.eq(argmax_preds, targets).float().sum() / np.prod(
                argmax_preds.shape
            )
            return acc
        else:
    ...
    def sample_from_dmol(self, outputs):
        logits, locs, log_scales, coeffs = self.split_to_dml_params(
            outputs, sampling=True
        )
        gumbel_noise = -torch.log(
            -torch.log(torch.rand_like(logits) * (1.0 - 2 * 1e-5) + 1e-5)
        )
        ...
        unif = torch.rand_like(log_scales) * (1.0 - 2 * 1e-5) + 1e-5
        ...
        x2 = torch.clamp(
            x[..., 2] + coeffs[..., 1] * x0 + coeffs[..., 2] * x1, -1.0, 1.0
        )
    ...
    def sample(self, n, device, argmax=False):
        ...
        for curr_infer_length in tqdm(range(total_len)):
            ...
            outputs = outputs.view(n, -1, outputs.shape[-1])[
                :, curr_infer_length : curr_infer_length + 1, :
            ]
            if self.hparams.distr == "dmol":
        ...
        samples = samples.view(
            n, self.hparams.image_size, self.hparams.image_size, self.hparams.channels
        )
...
class DecoderLayer(nn.Module):
    ...
    def __init__(self, hparams):
        ...
        self.layernorm_attn = nn.LayerNorm(
            [self.hparams.hidden_size], eps=1e-6, elementwise_affine=True
        )
        self.layernorm_ffn = nn.LayerNorm(
            [self.hparams.hidden_size], eps=1e-6, elementwise_affine=True
        )
        self.ffn = nn.Sequential(
            nn.Linear(self.hparams.hidden_size, self.hparams.filter_size, bias=True),
            nn.ReLU(),
...
class Attn(nn.Module):
    ...
    def forward(self, X):
        ...
        q = q.view(
            q.shape[:-1] + (self.hparams.num_heads, self.kd // self.hparams.num_heads)
        ).permute([0, 2, 1, 3])
        k = k.view(
            k.shape[:-1] + (self.hparams.num_heads, self.kd // self.hparams.num_heads)
        ).permute([0, 2, 1, 3])
        v = v.view(
            v.shape[:-1] + (self.hparams.num_heads, self.vd // self.hparams.num_heads)
        ).permute([0, 2, 1, 3])
        ...
        if self.hparams.attn_type == "global":
            ...
            pad = (
                ...
                0,
                0,
                (-len) % self.hparams.block_length,
            )  # Append to multiple of block length
            q = F.pad(q, pad)
            k = F.pad(k, pad)
            v = F.pad(v, pad)
            ...
            if q.shape[2] > blen:
                ...
                local_k = torch.cat(
                    [k[:, :, :-1], k[:, :, 1:]], 3
                )  # [batch, nheads, (nblocks - 1), blen * 2, depth]
                local_v = torch.cat([v[:, :, :-1], v[:, :, 1:]], 3)
                tail_q = q[:, :, 1:]
                bias = -1e9 * torch.triu(torch.ones(blen, 2 * blen), blen + 1).to(
                    X.device
                )
                tail_output = self.dot_product_attention(
                    tail_q, local_k, local_v, bias=bias
                )
                tail_output = tail_output.view(
                    tail_output.shape[0], tail_output.shape[1], -1, tail_output.shape[4]
                )
